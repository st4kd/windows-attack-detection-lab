...
- Detect suspicious PowerShell command-line execution
- Detect suspicious Certutil LOLBin activity
...
- [`sysmonconfig.xml`](sysmonconfig.xml) — tuned Sysmon configuration used to collect endpoint telemetry
- [`powershell-download.spl`](detections/powershell-download.spl) — detection query for PowerShell file creation activity
- [`run-key-persistence.spl`](detections/run-key-persistence.spl) — detection query for Windows Run-key persistence
- [`images/`](images/) — screenshots documenting telemetry tuning, attack simulation, detection, and investigation

Detection queries for suspicious PowerShell command execution and Certutil activity are documented directly in their respective sections above.

## Key Takeaways

- Effective detection engineering begins with useful telemetry, not simply collecting the highest possible volume of events.
- Sysmon logging should be reviewed and tuned based on observed event volume and investigative value.
- `ProcessGuid` can correlate multiple events generated by the same process and help reconstruct related activity.
- Sysmon Event ID 1 provides valuable process context, including the full command line, user, integrity level, and parent process.
- Raw XML telemetry can be transformed into concise, analyst-friendly results that make suspicious activity easier to investigate.
- PowerShell and legitimate Windows utilities such as `certutil.exe` can perform security-relevant actions, making behavior and execution context more important than the executable name alone.
- Parent-child process relationships provide useful context for distinguishing expected activity from execution that warrants further investigation.
- Safe attack simulation provides a repeatable way to validate detection logic without introducing malicious code into the lab environment.

---

## Conclusion

This project demonstrated an end-to-end Windows detection workflow: configuring and tuning endpoint telemetry, generating benign attack-like activity, building targeted Splunk searches, and investigating the resulting process, file, registry, and command-line evidence.

The completed lab includes detections for PowerShell file creation, Windows Run-key persistence, suspicious PowerShell command execution, and Certutil LOLBin activity.
